...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		...
		for (j=1 , k = 1; k <=((SIZE-2)/2);k++, j+=2 ) {
			 ...
			 * of the derivative.								  */
			resx += input[t1 + j -1] * horiz_operator[0][0];
			resx += input[t1 + j ] * horiz_operator[0][1];
			resx += input[t1 + j + 1] * horiz_operator[0][2];
			resx += input[t2 + j + -1] * horiz_operator[1][0];
			resx += input[t2 + j ] * horiz_operator[1][1];
			resx += input[t2 + j + 1] * horiz_operator[1][2];
			resx += input[t3 + j -1] * horiz_operator[2][0];
			resx += input[t3 + j ] * horiz_operator[2][1];
			resx += input[t3 + j + 1] * horiz_operator[2][2];
			
									
			resy += input[t1 + j -1] * vert_operator[0][0];
			resy += input[t1 + j ] * vert_operator[0][1];
			resy += input[t1 + j + 1] * vert_operator[0][2];
			resy += input[t2 + j -1] * vert_operator[1][0];
			resy += input[t2 + j ] * vert_operator[1][1];
			resy += input[t2 + j + 1] * vert_operator[1][2];
			resy += input[t3 + j  -1] * vert_operator[2][0];
			resy += input[t3 + j ] * vert_operator[2][1];
			resy += input[t3 + j + 1] * vert_operator[2][2];
			...
			/* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			resx += input[t1 + j] * horiz_operator[0][0];
			resx += input[t1 + j+1 ] * horiz_operator[0][1];
			resx += input[t1 + j + 2] * horiz_operator[0][2];
			resx += input[t2 + j] * horiz_operator[1][0];
			resx += input[t2 + j +1] * horiz_operator[1][1];
			resx += input[t2 + j +2] * horiz_operator[1][2];
			resx += input[t3 + j] * horiz_operator[2][0];
			resx += input[t3 + j + 1 ] * horiz_operator[2][1];
			resx += input[t3 + j + 2] * horiz_operator[2][2];
			
									
			resy += input[t1 + j] * vert_operator[0][0];
			resy += input[t1 + j +1] * vert_operator[0][1];
			resy += input[t1 + j + 2] * vert_operator[0][2];
			resy += input[t2 + j] * vert_operator[1][0];
			resy += input[t2 + j +1] * vert_operator[1][1];
			resy += input[t2 + j + 2] * vert_operator[1][2];
			resy += input[t3 + j ] * vert_operator[2][0];
			resy += input[t3 + j +1] * vert_operator[2][1];
			resy += input[t3 + j + 2] * vert_operator[2][2];
			...
			if (res > 255)
				...
				output[t2 + j + 1] = (unsigned char)res;
			//////////////////////////////////////////////////////////////////
			t = pow((output[t2+j] - golden[t2+j]),2);
			PSNR += t;
			t = pow((output[t2+j+1] - golden[t2+j+1]),2);
			PSNR += t;
