...
{
	double PSNR = 0, t;
	int i, j, k, resx, resy, t1, t2, t3, t4, t5, j1, j2, j3;
	unsigned int p;
	int res;
	...
	for (i=1; i<SIZE-1; i+=1) {
		...
		for (j=1 , k = 1; k <=t4;k++, j+=2 ) {
			 ...
			 * of the derivative.								  */
			resx += input[j1 -1] * horiz_operator[0][0];
			resx += input[j1 ] * horiz_operator[0][1];
			resx += input[j1 + 1] * horiz_operator[0][2];
			resx += input[j2 + -1] * horiz_operator[1][0];
			resx += input[j2 ] * horiz_operator[1][1];
			resx += input[j2 + 1] * horiz_operator[1][2];
			resx += input[j3 -1] * horiz_operator[2][0];
			resx += input[j3 ] * horiz_operator[2][1];
			resx += input[j3 + 1] * horiz_operator[2][2];
			
									
			resy += input[j1 -1] * vert_operator[0][0];
			resy += input[j1 ] * vert_operator[0][1];
			resy += input[j1 + 1] * vert_operator[0][2];
			resy += input[j2 -1] * vert_operator[1][0];
			resy += input[j2 ] * vert_operator[1][1];
			resy += input[j2 + 1] * vert_operator[1][2];
			resy += input[j3  -1] * vert_operator[2][0];
			resy += input[j3 ] * vert_operator[2][1];
			resy += input[j3 + 1] * vert_operator[2][2];
			...
			/* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			resx += input[j1] * horiz_operator[0][0];
			resx += input[j1+1 ] * horiz_operator[0][1];
			resx += input[j1 + 2] * horiz_operator[0][2];
			resx += input[j2] * horiz_operator[1][0];
			resx += input[j2 +1] * horiz_operator[1][1];
			resx += input[j2 +2] * horiz_operator[1][2];
			resx += input[j3] * horiz_operator[2][0];
			resx += input[j3 + 1 ] * horiz_operator[2][1];
			resx += input[j3 + 2] * horiz_operator[2][2];
			
									
			resy += input[j1] * vert_operator[0][0];
			resy += input[j1 +1] * vert_operator[0][1];
			resy += input[j1 + 2] * vert_operator[0][2];
			resy += input[j2] * vert_operator[1][0];
			resy += input[j2 +1] * vert_operator[1][1];
			resy += input[j2 + 2] * vert_operator[1][2];
			resy += input[j3 ] * vert_operator[2][0];
			resy += input[j3 +1] * vert_operator[2][1];
			resy += input[j3 + 2] * vert_operator[2][2];
			...
			if (res > 255)
				...
				output[j2 + 1] = (unsigned char)res;
			//////////////////////////////////////////////////////////////////
			t = pow((output[j2] - golden[j2]),2);
			PSNR += t;
			t = pow((output[j2+1] - golden[j2+1]),2);
			PSNR += t;
